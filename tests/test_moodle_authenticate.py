...
class TestMoodleAuthenticatedSession:
    ...
    def test_enter_calls_authenticate(self, monkeypatch):
        ...
        monkeypatch.setattr(session, 'authenticate_on_moodle', mock_authenticate)
        
        # Call __enter__
        session.__enter__()  # these params should be ignored
        
        # Verify authenticate was called with credentials from __init__
        assert auth_called, "authenticate_on_moodle was not called with correct credentials"
    ...
    def test_enter_returns_self(self, monkeypatch):
        ...
        assert result is session, "__enter__ should return the session object itself"
    ...
    def test_authenticate_on_moodle_success(self, monkeypatch):
        ...
        def mock_get(*args, **kwargs):
            ...
            mock_response._content = b"<html><body>Mock login page</body></html>"
    ...
    def test_authenticate_fail_login_post(self, monkeypatch):
        ...
        with pytest.raises(Exception, match="500 Failed to authenticate on login page"):
            session.authenticate_on_moodle("test_user", "test_pass")
    ...
    def test_authenticate_fail_saml_extraction(self, monkeypatch):
        ...
        def mock_post(url, **kwargs):
            ...
            if "login.php" in url:
                ...
                mock_response.url = "mock_url?param=value"
                mock_response._content = b'<html><body>Invalid credentials</body></html>'
    ...
    def test_authenticate_sends_correct_parameters(self, monkeypatch):
        ...
        assert get_url == "https://moodle.univ-ubs.fr/auth/shibboleth/login.php"
        assert post_urls[0] == "https://moodle.univ-ubs.fr/auth/shibboleth/login.php"
        assert post_data[0] == {"idp": "urn:mace:cru.fr:federation:univ-ubs.fr"}
        assert post_data[1] == {
            "username": "test_user",
            "password": "test_pass",
